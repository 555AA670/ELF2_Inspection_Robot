...
static const uint8_t OLED_I2C_AddressList[] = {0x78, 0x7A};
...
void OLED_WriteCommand(uint8_t Command)
{
	...
	for (i = 0; i < sizeof(OLED_I2C_AddressList); i++)
	{
		OLED_I2C_Start();							//I2C起始
		OLED_I2C_SendByte(OLED_I2C_AddressList[i]);	//发送OLED的I2C从机地址
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	for (addrIndex = 0; addrIndex < sizeof(OLED_I2C_AddressList); addrIndex++)
	{
		OLED_I2C_Start();								//I2C起始
		OLED_I2C_SendByte(OLED_I2C_AddressList[addrIndex]);	//发送OLED的I2C从机地址
